...
def tool_retrieve_company_info():
    ...
    """
    return Tool(name="Get info from company", func=lambda query: get_company_info(user_reply=query, index_path=company_handbook_faiss_path), description=f"""useful for questions about GitLab's polices, work from home, IT,  CEO, product, meeting conduct,, diversity and inclusion (DEI), career progression, management tips, sales process, finance, HR, accounting, team events, 1 on 1 tips, coaching tips and finance. Pass user's response directly to this tool""", return_direct=True)  # type: ignore
